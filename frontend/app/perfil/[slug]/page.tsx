    ...
    <div className="container-app py-14">
      <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
        <Surface className="sticky top-24 h-fit">
          <div className="flex flex-col items-center text-center">
            <div className="h-28 w-28 overflow-hidden rounded-[26px] border border-white/10 bg-slate-900">
              {profile.avatarUrl ? <img src={profile.avatarUrl} alt={profile.name} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-4xl font-black text-cyan-200">{profile.name.slice(0, 1)}</div>}
            </div>
        ...
        <div className="grid gap-6">
          <Surface>
            <div className="text-sm font-black uppercase tracking-[0.18em] text-cyan-200">Sobre</div>
          ...
          <Surface>
            <div className="text-sm font-black uppercase tracking-[0.18em] text-cyan-200">Especialidades</div>
            <div className="mt-4 flex flex-wrap gap-3">{profile.specialties.map((item) => <span key={item.id} className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm font-semibold text-slate-200">{item.name}</span>)}</div>
          </Surface>

          {profile.photos.length ? (
            <Surface>
              <div className="text-sm font-black uppercase tracking-[0.18em] text-cyan-200">Galeria</div>
          ...
          <Surface>
            <div className="text-sm font-black uppercase tracking-[0.18em] text-cyan-200">Cidades atendidas</div>
          ...
          <Surface>
            <div className="text-sm font-black uppercase tracking-[0.18em] text-cyan-200">Avaliações</div>
